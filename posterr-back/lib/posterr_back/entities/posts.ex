defmodule PosterrBack.Entities.Posts do
  ...
  Creates a new post
  """
  @spec create(attrs :: map(), user_id :: integer) :: {:ok, Post.t()} | {:error, Ecto.Changeset.t()}
  ...
  def list_posts_with_reposts(page_number, page_size) when is_integer(page_number) and is_integer(page_size) do
    from(p in Post,
  ...
  def count_user_posts_and_reposts_for_a_day(user_id, date) do
    ...
    post_count = Repo.aggregate(from(p in Post,
      where: p.user_id == ^user_id,
      where: p.inserted_at >= ^start_of_day,
      where: p.inserted_at <= ^end_of_day,
      select: count(p.id)), :count)

    repost_count = Repo.aggregate(from(r in Repost,
  ...
  def list_posts_sorted("latest", page_number, page_size), do: list_posts_with_reposts(page_number, page_size)
